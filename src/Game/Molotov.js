var Molotov = cc.Sprite.extend({
    ...
    ctor:function (x, speed, unit_tag) {
        ...
        var molotov_sprite = cc.SpriteFrameCache.getInstance().addSpriteFrames(s_plist_item_molotov);

        this.tag = unit_tag;

        var pFrame = cc.SpriteFrameCache.getInstance().getSpriteFrame("molotov_0000");
        ...
        var str = "";
        for (var i = 0; i < 3; i++) {
            str = "molotov_00" + (i < 10 ? ("0" + i) : i) + "";
            var frame = cc.SpriteFrameCache.getInstance().getSpriteFrame(str);
            animFrames.push(frame);
        }
        var molotov_animation = cc.Animation.create(animFrames, 0.2);

        this.runAction( 
            cc.RepeatForever.create( 
                cc.Sequence.create(
                    cc.Animate.create(molotov_animation)
                )
            )
        );
